...
export interface PersistentSubscriptionSettingsGeneric {
  ...
  /**
   * The amount of time in milliseconds after which a message should be
   * considered to be timeout and retried.
   *
   * @default 30_000
   */
  ...
  /**
   * The maximum number of retries (due to timeout) before a message gets
...
export interface PersistentSubscriptionToStreamSettings
  ...
  /**
   * Where to start the subscription from. This can be from the start of the
   * stream, from the end of the stream at the time of creation, or from an
   * inclusive position in the stream.
   *
   * @default END
   */
